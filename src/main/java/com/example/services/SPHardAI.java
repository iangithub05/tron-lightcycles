...
/**
 * Singleplayer Hard AI: hunts the human relentlessly while also surviving.
 * Deep 45-step lookahead lets it cut ahead of the human's path.
 * High space weight means it won't sacrifice itself — it stays alive AND kills.
 * Zero noise: every decision is optimal.
 */
public class SPHardAI implements AIController {
    ...
    private static final int MAX_FLOOD = 5000;
    private static final int LOOK_AHEAD = 45;

    private static final double SPACE_WEIGHT     = 1.0;
    private static final double INTERCEPT_WEIGHT = 160.0;
    private static final double MOMENTUM         = 1.25;
    ...
    public Direction computeDirection(Player player, Game game) {
        ...
        // Far lookahead: intercept where the human will be 45 steps from now
        ...
        for (Direction d : Direction.values()) {
            ...
            // Full survival weight AND very high intercept: hunts efficiently without dying
            double score = space * SPACE_WEIGHT
                         + (distBefore - distAfter) * INTERCEPT_WEIGHT;

            if (d == player.direction) score *= MOMENTUM;
            // No noise: zero mistakes, fully deterministic
